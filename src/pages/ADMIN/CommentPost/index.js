...
const CommentPost = () => {
  ...
  const columns = [
    ...
    {
      name: "Bài viết",
      element: (row) => row.ownership_post.title,
    },
    {
      name: "Bình luận",
  ...
  const requestDeleteApi = () => {
    if (deleteType === "single") {
      ...
      requestApi(`/comment-posts/${deleteItem}`, "DELETE", [])
        .then((response) => {
          setShowModal(false);
          setRefresh(Date.now());
      ...
      requestApi(`/comment-posts/multiple-delete`, "DELETE", ids)
        .then((response) => {
          setShowModal(false);
          setRefresh(Date.now());
          setSelectedRows([]);
  ...
  useEffect(() => {
    ...
    requestApi(
      `/comment-posts/get-full-comments-in-post/${params.id}${query}`,
      "GET",
      []
    )
      .then((response) => {
